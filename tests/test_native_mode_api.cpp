...
{
    QJsonValue result = callResult("qt.objects.find",
        QJsonObject{{"name", "testBtn"}});
    QVERIFY(result.isObject());
...
{
    QJsonObject error = callExpectError("qt.objects.find",
        QJsonObject{{"name", "nonexistent_widget_xyz"}});
...
{
    QJsonValue result = callResult("qt.objects.findByClass",
...
{
    QJsonValue result = callResult("qt.objects.tree",
        QJsonObject{{"maxDepth", 2}});
    QVERIFY(result.isObject());

    QJsonObject tree = result.toObject();
    // Tree should have some structure - at least children or className
    QVERIFY(tree.contains("children") || tree.contains("className")
            || tree.contains("id"));
}
...
void TestNativeModeApi::testObjectsInfo()
{
    ...
    QJsonValue findResult = callResult("qt.objects.find",
        QJsonObject{{"name", "testBtn"}});
    QString objectId = findResult.toObject()["objectId"].toString();
    QVERIFY(!objectId.isEmpty());

    // Now get info
    QJsonValue result = callResult("qt.objects.info",
...
void TestNativeModeApi::testObjectsInspect()
{
    ...
    // Inspect
    QJsonValue result = callResult("qt.objects.inspect",
        QJsonObject{{"objectId", objectId}});
    QVERIFY(result.isObject());
    ...
    QCOMPARE(inspected["info"].toObject()["className"].toString(),
             QString("QPushButton"));
...
void TestNativeModeApi::testObjectsQuery()
{
    ...
    QJsonValue result = callResult("qt.objects.query",
        QJsonObject{{"className", "QPushButton"}});
    QVERIFY(result.isArray());
...
void TestNativeModeApi::testObjectsQueryWithPropertyFilter()
{
    ...
    QJsonValue result = callResult("qt.objects.query",
        QJsonObject{
            ...
            {"properties", QJsonObject{{"enabled", false}}}
        });
    QVERIFY(result.isArray());
...
void TestNativeModeApi::testPropertiesList()
{
    ...
    QJsonValue result = callResult("qt.properties.list",
        QJsonObject{{"objectId", objectId}});
    QVERIFY(result.isArray());
...
void TestNativeModeApi::testPropertiesGetSet()
{
    ...
    // Get text
    QJsonValue getResult = callResult("qt.properties.get",
        QJsonObject{{"objectId", objectId}, {"name", "text"}});
    QVERIFY(getResult.isObject());
    QCOMPARE(getResult.toObject()["value"].toString(), QString("Test Button"));

    // Set text
    QJsonValue setResult = callResult("qt.properties.set",
    ...
    QJsonValue getResult2 = callResult("qt.properties.get",
        QJsonObject{{"objectId", objectId}, {"name", "text"}});
...
void TestNativeModeApi::testMethodsList()
{
    ...
    QJsonValue result = callResult("qt.methods.list",
        QJsonObject{{"objectId", objectId}});
    QVERIFY(result.isArray());
...
void TestNativeModeApi::testMethodsInvoke()
{
    ...
    QJsonValue result = callResult("qt.methods.invoke",
        QJsonObject{
            {"objectId", objectId},
            {"method", "setEnabled"},
            {"args", QJsonArray{false}}
        });
    QVERIFY(result.isObject());
...
void TestNativeModeApi::testSignalsList()
{
    ...
    QJsonValue result = callResult("qt.signals.list",
        QJsonObject{{"objectId", objectId}});
    QVERIFY(result.isArray());
...
void TestNativeModeApi::testSignalsSubscribeUnsubscribe()
{
    ...
    // Unsubscribe
    QJsonValue unsubResult = callResult("qt.signals.unsubscribe",
        QJsonObject{{"subscriptionId", subscriptionId}});
...
void TestNativeModeApi::testUiGeometry()
{
    ...
    QJsonValue result = callResult("qt.ui.geometry",
        QJsonObject{{"objectId", objectId}});
    QVERIFY(result.isObject());
...
void TestNativeModeApi::testUiScreenshot()
{
    ...
    QJsonValue result = callResult("qt.ui.screenshot",
        QJsonObject{{"objectId", objectId}});
    QVERIFY(result.isObject());
...
void TestNativeModeApi::testUiClick()
{
    ...
    QJsonValue result = callResult("qt.ui.click",
        QJsonObject{{"objectId", objectId}});
    QApplication::processEvents();
...
void TestNativeModeApi::testUiSendKeys()
{
    ...
    QJsonValue result = callResult("qt.ui.sendKeys",
        QJsonObject{{"objectId", objectId}, {"text", "Hello"}});
    QApplication::processEvents();
...
void TestNativeModeApi::testNamesUnregister()
{
    ...
    callResult("qt.names.register",
        QJsonObject{{"name", "tempName"}, {"path", "some/path"}});
    ...
    // Unregister
    QJsonValue unregResult = callResult("qt.names.unregister",
        QJsonObject{{"name", "tempName"}});
...
void TestNativeModeApi::testNamesValidate()
{
    ...
    callResult("qt.names.register",
        QJsonObject{{"name", "validBtn"}, {"path", validPath}});
...
void TestNativeModeApi::testNumericIdResolution()
{
    ...
    QJsonValue infoResult = callResult("qt.objects.info",
        QJsonObject{{"objectId", numericRef}});
...
void TestNativeModeApi::testSymbolicNameResolution()
{
    ...
    callResult("qt.names.register",
        QJsonObject{{"name", "symBtn"}, {"path", hierPath}});

    // Now call qt.objects.info using the symbolic name
    QJsonValue infoResult = callResult("qt.objects.info",
        QJsonObject{{"objectId", "symBtn"}});
    QVERIFY(infoResult.isObject());
    QCOMPARE(infoResult.toObject()["className"].toString(), QString("QPushButton"));
}
...
void TestNativeModeApi::testStructuredErrorMissingObjectId()
{
    ...
    QJsonObject error = callExpectError("qt.properties.get",
        QJsonObject{{"name", "text"}});
...
void TestNativeModeApi::testStructuredErrorObjectNotFound()
{
    ...
    QJsonObject error = callExpectError("qt.objects.info",
        QJsonObject{{"objectId", "nonexistent/path/xyz"}});
