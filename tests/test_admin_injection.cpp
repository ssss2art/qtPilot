...
void TestAdminInjection::initTestCase()
{
    ...
    if (m_launcherPath.isEmpty()) {
        ...
        QDir exeDir(QCoreApplication::applicationDirPath());
        QStringList matches = exeDir.entryList(
            {QStringLiteral("qtmcp-launch*")}, QDir::Files, QDir::Name);
        if (!matches.isEmpty())
            m_launcherPath = exeDir.absoluteFilePath(matches.first());
    }
    QVERIFY2(!m_launcherPath.isEmpty(),
             "Could not find qtmcp-launch executable in build output");
    ...
    QVERIFY2(!m_testAppPath.isEmpty(),
             "Could not find test app executable in build output");
...
void TestAdminInjection::testElevatedLaunchAndConnect()
{
    ...
    launcher.start();
    QVERIFY2(launcher.waitForFinished(30000),
             "Launcher did not finish within 30 seconds");
    ...
    QString wsUrl;
    {
        ...
        for (int attempt = 0; attempt < 30 && !discovered; ++attempt) {
            ...
            while (udp.hasPendingDatagrams()) {
                ...
                if (msg.value(QStringLiteral("type")).toString()
                        == QStringLiteral("announce")
                    && msg.value(QStringLiteral("protocol")).toString()
                           == QStringLiteral("qtmcp-discovery")) {
                    ...
                    qDebug() << "Discovered probe: pid" << m_targetPid
                             << "port" << wsPort;
                    break;
                }
            }
        ...
        QVERIFY2(discovered,
                 "Did not receive probe discovery broadcast within 15 seconds");
    }
    ...
    // 5. Send chr.getPageText and verify a response arrives
    {
        ...
        QVERIFY2(messageSpy.wait(10000),
