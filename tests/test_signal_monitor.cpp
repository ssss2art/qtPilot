...
void TestSignalMonitor::testLifecycleCreated() {
    ...
    QVERIFY2(spy.count() >= 1, qPrintable(QString("Expected >= 1 created events, got %1").arg(spy.count())));
...
void TestSignalMonitor::testLifecycleDestroyed() {
    ...
    QVERIFY2(spy.count() >= 1, qPrintable(QString("Expected >= 1 destroyed events, got %1").arg(spy.count())));
