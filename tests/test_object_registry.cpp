...
// The marker present but the index absent or not a number. Nothing here was
// generated by the QML engine, so the whole name is the declared name.
class Probe_QMLTYPE_x : public QObject {
  Q_OBJECT
};

class Probe_QML_9z : public QObject {
  Q_OBJECT
};
...
void TestObjectRegistry::testDeclaredNameDoesNotMatchAnUnrelatedType() {
  ...
  // The index has to be a number running to the end of the name. A marker
  // followed by anything else was not generated, so none of it may be shed --
  // otherwise two distinct types collapse onto one name.
  auto* markerOnly = new Probe_QMLTYPE_x();
  markerOnly->setParent(parent.get());
  auto* trailingText = new Probe_QML_9z();
  trailingText->setParent(parent.get());

  QCoreApplication::processEvents();

  QEXPECT_THAT(registry->findAllByClassName(QStringLiteral("Probe"), parent.get()),
               AllOf(Not(Contains(markerOnly)), Not(Contains(trailingText))));
}
